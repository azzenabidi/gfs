...
export type Served = "source" | "remote" | "local";
...
export interface Stats {
  ...
  localReviews?: number | null;
  cachedRanges?: number | null;
  fullyCached?: number | null;
}
...
export const api = {
  ...
  warm: (table: string, lo: number, hi: number) =>
    json<{ hydrated: number }>("/api/warm", { method: "POST", body: JSON.stringify({ table, lo, hi }) }),
